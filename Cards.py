...
MTG_CARD_MAX_AREA = 100000
MTG_CARD_MIN_AREA = 15000
...
def find_cards(thresh_image):
	...
	image = Cards.preprocess_image(image)

	# Find contours and sort their indices by contour size
	dummy,cnts,hier = cv2.findContours(image,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
...
def find_MTG_cards(mtg_image, thresh = 200):
	"""Finds all card-sized contours in a thresholded camera image.
	Returns the number of cards, and a list of card contours sorted
	from largest to smallest."""
	# Edge detection using canny filter
#	edges = image.clone() #cv.CloneImage(grey_image) # Pre-process camera image (gray, blur, and threshold it)
	canny_image = cv2.Canny(preprocess_image(mtg_image),thresh, thresh)
	# Find contours and sort their indices by contour size
#	dummy,cnts,hier = cv2.findContours(thresh_image, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
	dummy, cnts, hier = cv2.findContours(canny_image, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
	...
	for i in range(len(cnts_sort)):
		...
		if ((size < MTG_CARD_MAX_AREA) and (size > MTG_CARD_MIN_AREA)
			and (hier_sort[i][3] == -1) and (len(approx) == 4)):
			cnt_is_card[i] = 1
...
def preprocess_MTGcard(contour, image):
	"""Uses contour to find information about the query card. Isolates rank
	and suit images from the card."""
...
# Grab detected card and adjust perspective for recognition.
